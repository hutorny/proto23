...
private:
    ...
    std::ostringstream             specializations_{};
    std::string                    ns_{};
    ...
    void emit_enum(std::ostream& out, const pb::EnumDescriptorProto& e, int depth, const std::string& prefix) {
        out << ind(depth) << deprecated(e.options) << "enum class " << e.name << sanitizing_suffix(e.name) << " : std::int32_t {\n";
        int min_val = std::numeric_limits<int>::max();
        int max_val = std::numeric_limits<int>::min();
        unsigned mask = 0;
        int next = e.value.empty() ? 0 : e.value[0].number;
        bool is_seq = true;
        for (const auto& v : e.value) {
            const auto val = static_cast<int>(v.number);
            min_val = std::min(min_val, val);
            max_val = std::max(max_val, val);
            mask |= static_cast<unsigned>(val);
            is_seq = is_seq && (val == next++);
            out << ind(depth + 1) << deprecated(v.options)
                << v.name << sanitizing_suffix(v.name) << " = "
                << static_cast<std::int32_t>(v.number) << ",\n";
        }
        out << ind(depth) << "};\n\n";
        std::string fqname = prefix + e.name;
        fqname += sanitizing_suffix(e.name);
        if( e.value.empty()) {
            specializations_ << "template<>\nstruct enum_traits<" << fqname << "> : "
                             << "proto23::detail::make_enum_traits<" << fqname << ",0,0,0,0,false>{};\n";
        } else {
            specializations_ << "template<>\nstruct enum_traits<" << fqname << "> : \n"
                             << ind(1) << "proto23::detail::make_enum_traits<" << fqname << ','
                             << min_val << ',' << max_val << ',' << e.value.size() << ',' << mask << "U,"
                             << (is_seq ? "true" : "false") << ">{};\n";
        }
    }
    ...
    void emit_message(std::ostream&                 out,
        ...
        for (const auto& e : msg.enum_type) {
            emit_enum(out, e, depth + 1, ns_ + "::" + child_prefix);
        }
...
std::string Generator::generate() {
    ...
    // --- Namespace open ---
    ns_ = pkg.empty() ? std::string{} : pkg_to_ns(pkg);
    ...
    // --- Namespace close ---
    if (!ns_.empty()) { out << "} // namespace " << ns_ << "\n"; }
    if (!specializations_.view().empty()) {
        out << "\nnamespace proto23 {\n\n" << specializations_.view() << "\n}\n";
    }
    return out.str();
}
